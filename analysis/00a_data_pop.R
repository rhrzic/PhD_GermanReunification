...
# Mortality rate (assuming end-year population has been constant)
...
mid_year_population <- population_reformed %>%
  ...
  mutate(pop = rollmean(pop_new, k = 2, align = "right", fill = NA)) %>%
...
saveRDS(mid_year_population, file = "temp_data/mid_year_pop.rds")
