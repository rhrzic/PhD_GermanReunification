...
hospitals1 <- read.csv(paste0("raw_data/healthcare/23111-01-02-4.csv"), header = FALSE, sep = ";", stringsAsFactors = F,
  ...
  slice(., 1:(n()-4)) %>%
  select(year = V1, AGS = V2, geo = V3, hospitals = V4, beds = V5)
...
hospitals_reformed <- hospitals %>%
  ...
  mutate(hospitals_new = ifelse(is.na(hospitals) & AGS %in% reformed_areas, sum(hospitals, na.rm = T), hospitals),
         beds_new = ifelse(is.na(beds) & AGS %in% reformed_areas, sum(beds, na.rm = T), beds)) %>%
  filter(!(AGS %in% redundant_areas)) %>%
  select(year, AGS = code_new, geo, hospitals = hospitals_new, beds = beds_new)

hospitals <- hospitals_reformed %>%
  ...
  summarise(hospitals = sum(hospitals),
            beds = sum(beds)) %>%
  ungroup()

saveRDS(hospitals, "temp_data/healthcare.rds")
